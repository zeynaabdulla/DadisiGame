...
TextSprite.prototype.getOpacity = Sprite.prototype.getOpacity;
TextSprite.prototype.setOpacity = Sprite.prototype.setOpacity;
 ...
 */
TextSprite.prototype.setFirstLine = function(firstLine)
{
    this._firstLine = firstLine || 0;
    !this._fixedSize && this._updateSize();
    this._image = 0;
};
 ...
 */
TextSprite.prototype.getFirstLine = function()
{
...
TextSprite.prototype._updateSize = function()
{
    ...
    // see how many lines we have to draw
    ...
    document.body.appendChild(TextSprite.textMeasuring.div); // this is a performance hit, but unavoidable
    ...
    var singleLineHeight = TextSprite.textMeasuring.block.getBoundingClientRect().top - textTop;
    TextSprite.textMeasuring.block.style.verticalAlign = 'baseline';
    if (!singleLineHeight)
    {
        singleLineHeight = context.measureText('m').width * 3 + this._outlineWidth;
    }
    this._centerOffset.y = singleLineHeight / 2 + (textTop - TextSprite.textMeasuring.block.getBoundingClientRect().top);
    document.body.removeChild(TextSprite.textMeasuring.div);
    this._lineHeight = singleLineHeight;
    ...
    this._size.x = width;
    this._size.y = singleLineHeight + (this._numLines - 1) * this._lineSpacing * singleLineHeight;
    this._centerOffset.y += (this._size.y - singleLineHeight) / 2;
    ...
    // apply user-defined scaling, for fonts that don't play by the rules and need manual tweaking
...
TextSprite.prototype._refresh = function(draw, measure, _context)
{
    ...
    else
    {
        ...
        for (var i=0; i<separateLines.length && (!this._maxLines || currentLine < this._maxLines); i++)
        {
            ...
            {
                var str = words.slice(0,wordIndex).join(' ');
                ...
                if (w > this._maxWidth && this._maxWidth > 0)
                {
                    ...
                    if ((!this._maxLines || currentLine < this._maxLines) && currentLine >= this._firstLine)
                    {
                        if (draw)
                        {
                            var posY = this._position.y + (this._lineHeight * this._lineSpacing * (currentLine - this._firstLine));
                            context.fillText(text, this._position.x, posY);
                            if (this._outlineWidth)
                            {
                                context.strokeText(this._text, this._position.x, posY);
                            }
                        }
                        if (measure)
                        {
            ...
            if (wordIndex > 0 && (!this._maxLines || currentLine < this._maxLines))
            {
                text = words.join(' ');
                if (draw)
                {
                    posY = this._position.y + (this._lineHeight * this._lineSpacing * (currentLine - this._firstLine));
                    context.fillText(text, this._position.x, posY);
                    if (this._outlineWidth)
                    {
                        context.strokeText(this._text, this._position.x, posY);
